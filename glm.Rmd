...
model_data
# subset minute-by-minute rows for that match/team
newdata <- model_data[match_id == test_match & team == test_team]

# if model used factors/splines, predict() will handle them if the model contains them
# 1) per-minute predicted probabilities
newdata$pred_p <- predict(goal_model, newdata = newdata, type = "response")

# 2) approximate 95% CI for p (on link scale then backtransform)
lp <- predict(goal_model, newdata = newdata, type = "link", se.fit = TRUE)
eta <- lp$fit
se_eta <- lp$se.fit
lower_eta <- eta - 1.96 * se_eta
upper_eta <- eta + 1.96 * se_eta
# inverse-logit (logit link):
inv_logit <- function(x) plogis(x)
newdata$p_lo  <- inv_logit(lower_eta)
newdata$p_hat <- inv_logit(eta)
newdata$p_hi  <- inv_logit(upper_eta)

# 3) expected goals in the match (sum of per-minute p)
exp_goals_total <- sum(newdata$pred_p, na.rm = TRUE)